...
> Version: v3.2 | Last updated: 2026-06-17
...
- **Secondary (墨蓝)**: Info pills, secondary indicators.
...
- **Semantic colors**: success=green for OK/completed, warning=amber for pending/alerts, danger=red for errors/failed.
...
line-height:
  tight:  1.3    # headings
  base:   1.55   # body
  compact: 1.45  # compact message bubbles
  loose:  1.7    # spacious text
```
...
chat:
  ...
  message:
    ...
    bubble:
      ...
      border:       1px solid var(--line)
      border-radius: 14px
      padding:      10px 14px          # compacted from 12px 16px
      font-size:    13px               # compacted from 14px
      line-height:  1.45              # compacted via --lh-compact
      box-shadow:   var(--sh-1)
  ...
  input-area:
    display:        flex              # redesigned: flex layout
    ...
    prompt-textarea:
      ...
      background:   var(--bg-soft)
      border-radius: var(--r)         # 10px
      font-family:  var(--font-mono)  # monospace prompt
      font-size:    13px
      color:        var(--ink)
      min-height:   36px
      max-height:   160px
      padding:      8px 12px
      resize:       vertical
      placeholder:  "color: var(--ink-faint); font-family: var(--font-sans)"
    ...
    upload-button:
      ...
      cursor:       pointer
      flex-shrink:  0
      hover:        "background: var(--bg-soft); color: var(--ink); border-color: var(--line-strong)"
...
```yaml
routes:
  /workbench:       "Workbench — agent chat interface, 3 columns (sidebar+content+inspector)"
  /files:           "FileManager — unified file management (knowledge/artifacts/memory), 2 columns"
  /packet:          "PacketAnalysis — TCP packet analysis, 2 columns"
  /runs:            "RunsPage — run history, traces, tool invocations, 2 columns"
  /jobs:            "JobsPage — scheduled & batch job management, 2 columns"
  /capabilities:    "CapabilityCenter — capability registry & matrix, 2 columns"
  /diagnostics:     "Diagnostics — system health, logs, metrics, 2 columns"
  /settings:        "Settings — LLM configuration, preferences, 2 columns"
  /:                "Redirect → /workbench"
```
...
tokens:
  ...
  line-height:      [--lh-tight: 1.3, --lh-base: 1.55, --lh-compact: 1.45, --lh-loose: 1.7]
