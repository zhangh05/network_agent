...
> **v0.9 — Artifact Consumption & Review Flow**（当前基线）：把后端能力链搭完整——新增 **`artifact`** capability（`artifact.list` / `artifact.read` / `artifact.diff` / `artifact.export`）+ **`review`** capability（`review.list_items` / `review.update_item`）。**不**做前端 API 对齐，**不**启用 topology / inspection / cmdb，**不**接真实设备；`translated_config` 仍是 `authoritative=false / deployable_config=false`。Tool count: **57 → 62**（v0.9 计划 +6，但 `artifact.list` 与 ToolRuntime catalog 已有项去重，实际净增 +5）。详见 [docs/ARTIFACT_REVIEW_FLOW_V09.md](docs/ARTIFACT_REVIEW_FLOW_V09.md)。
>
> **v0.8.2 — Result Contract Standardization**（前一基线）：统一结果链路 `ModuleResult → ToolResult → AgentResult.tool_calls`。
>
> **v0.8.1 — SkillSelector + Dynamic Tool Visibility**（更前基线）。
> 详见 [docs/CAPABILITY_MANIFEST_V08.md](docs/CAPABILITY_MANIFEST_V08.md) § 9 / § 10。

## Platform Runtime — Current Baseline

- **Agent Backend v0.9 — Artifact Consumption & Review Flow**
- **HEAD**：(see git log — v0.9 feat(agent): add artifact consumption and review flow)
- **Runtime architecture**：Codex-style Agent Runtime (Thread / Session / Turn / RuntimeLoop)
- **Tool count**：**62** (v0.6.x: 55 → v0.7: 57 → v0.9: 62，+5)
...
| v0.6.x ~ v0.9 broader focused regression | **715** | 7 | 0 | 7 skipped = `RUN_LIVE_TESTS=1` live LLM tests. v0.7.1 baseline 615 + v0.8 新增 20 + v0.8.1 新增 23 + v0.8.2 新增 28 + v0.9 新增 29. **0 failed**. |
