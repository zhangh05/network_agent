...
> **v1.0.1 — Document Ingestion & Book Library**（当前基线）：把 `knowledge` capability 升级为**完整后端知识库能力**——原始文件 → 标准化 Markdown → Source → Parent/Child Chunks → BM25 混合检索 → 父级上下文扩展 → 真实来源返回。新增 6 个 knowledge tool（`import_file` / `list_chunks` / `search_chunks` / `read_chunk` / `read_parent` / `reindex_source`）；保留 v1.0 6 个 + `knowledge.query` 改为高层封装（3 段 fallback：chunk→v1.0 store→legacy loader）。**不**依赖外部 DB（纯 BM25 + JSONL）；**不**伪造章节/页码/分数；**不**把整本书直接塞进 LLM；扫描型 PDF 返回 `unsupported_ocr`。Tool count: **67 → 73**（+6）。详见 [docs/DOCUMENT_INGESTION_BOOK_LIBRARY_V101.md](docs/DOCUMENT_INGESTION_BOOK_LIBRARY_V101.md)。
>
> **v1.0 — Knowledge Store Management**（前一基线）：详见 [docs/KNOWLEDGE_STORE_V10.md](docs/KNOWLEDGE_STORE_V10.md)。
>
...
- **Agent Backend v1.0.1 — Document Ingestion & Book Library**
- **HEAD**：(see git log — v1.0.1 feat(agent): add document ingestion and book library)
- **Runtime architecture**：Codex-style Agent Runtime (Thread / Session / Turn / RuntimeLoop)
- **Tool count**：**73** (v0.6.x: 55 → v0.7: 57 → v0.9: 62 → v1.0: 67 → v1.0.1: 73，+6)
