...
Windows 用户应从 GitHub Release 下载 `network-agent-<version>-windows-x64.zip`，完整解压后双击 `start.bat`。脚本会创建项目专用 `.venv`、按锁文件安装依赖、构建或复用前端并打开浏览器。不要使用 GitHub 自动生成的 `Source code` 压缩包作为 Windows 安装包。完整说明见 [Windows 使用指南](docs/WINDOWS.md)。
