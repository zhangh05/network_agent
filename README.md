...
> **v1.0.2 — Retrieval Quality & Evaluation**（当前基线）：在 v1.0.1.1 之上做**中文检索质量**与**可复现评测体系**的硬升级（**不**新增业务能力）。CJK 2-gram + 3-gram 混合分词（无字典）；字段加权 BM25（title 2.0 / chapter 1.5 / section 1.2 / tags 1.2 / body 1.0）；`KNOWLEDGE_BM25_K1` / `B` 环境变量可调；确定性查询扩展（OSPF↔开放式最短路径优先 / BGP↔边界网关协议 / DR / BDR / 邻居 / 邻接 等 60+ 项；**不**调 LLM；全部记录在 `metadata.query_expansions`）；兄弟 chunk 去重（同一 source_id + Jaccard≥0.85 → 只保留最高分；跨源 / 跨章节不破坏）；H3 子章节入 metadata（`subsection`）；`metadata.tokenizer_version=v1_cjk_ngram` / `scoring_version=v1_bm25_field_weighted`；**新增** 5 文档 / 15 查询评测集 + 评测脚本 + 19 个测试（含门禁测试，子进程跑 `evaluate_retrieval_v102.py` 并断言 4 项阈值）。**当前结果**：Recall@1=0.7333 / Recall@3=0.8667（≥0.85）/ Recall@5=0.9333 / MRR=0.8167（≥0.75）/ no_hit_precision=1.0 / duplicate_rate=0.0。Tool count 仍为 **73**（无新增工具）。详见 [docs/RETRIEVAL_QUALITY_V102.md](docs/RETRIEVAL_QUALITY_V102.md)。
>
...
- **Agent Backend v1.0.2 — Retrieval Quality & Evaluation**
- **HEAD**：(see git log — v1.0.2 feat(knowledge): improve retrieval quality and evaluation)
- **Runtime architecture**：Codex-style Agent Runtime (Thread / Session / Turn / RuntimeLoop)
- **Tool count**：**73** (v0.6.x: 55 → v0.7: 57 → v0.9: 62 → v1.0: 67 → v1.0.1: 73 → v1.0.1.1: 73 → v1.0.2: 73，**+0**)
