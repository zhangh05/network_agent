...
> **Frontend v1.0 — Capability-driven Agent Workbench Rewrite**（当前基线）：废弃旧单文件前端，重写为 **React 18 + TypeScript 5 + Vite 5 + React Router 6 + Zustand 4** 的 Capability-driven Agent Workbench。**不**引入 Material-UI / Ant Design / Chakra 等大型 UI 框架；**不**硬编码 Tool count / capability 状态；planned capability **不**渲染调用按钮；三栏布局（左 Workspace/Sessions/Runs · 中 Agent 对话与结果 · 右 Turn Inspector）；7 个 page 覆盖 Agent / Knowledge / Artifacts / Reviews / Capabilities / Runtime Audit / Settings；9 个 TypeScript 类型严格映射后端 dataclass `as_dict()`（**不**在组件中猜字段）；10 个 axios API 模块（agent / sessions / workspaces / capabilities / tools / knowledge / artifacts / reviews / runtime_audit / settings），错误统一转 `ApiError`；10 个 Vitest 测试覆盖 10 类断言。**Gates**：typecheck / build / test 全部 PASS（13/13 tests）。后端 0 改动；Tool count 仍为 **73**；planned 仍 0 可见；不接真实设备；不开 SSH·Telnet·SNMP·nmap；`config.push` 永久禁止。旧单文件前端保留为 `frontend/legacy/index.html.legacy`（legacy 备份，不继续扩展）。详见 [docs/FRONTEND_V1.md](docs/FRONTEND_V1.md)。
>
...
| v1.0 frontend tests | **13 / 13** | 0 | 0 | `frontend/src/test/*.test.tsx` — 10 个 Vitest 文件覆盖 10 类断言（agentResult / toolCalls / artifactCard / sourceSummary / reviewItem / plannedCapability / apiError / emptyState / sessionSwitch / inspectorToggle）；npm `typecheck` / `build` / `test` 三门禁全过；后端 0 改动；Tool count 仍 73 |
