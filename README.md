...
> **v0.8.2 — Result Contract Standardization**（当前基线）：统一结果链路 `ModuleResult → ToolResult → AgentResult.tool_calls`，让所有能力输出结构一致。`ModuleResult` 是业务输出合同；`ToolResult` 是 Runtime/LLM 工具结果合同；`AgentResult.tool_calls` 是前端/审计展示合同。v0.7.1 业务输出合同（`translate_config` / `query_knowledge` 的 dict 返回）一字不动；capability tests 41/41 零回归。
>
> **v0.8.1 — SkillSelector + Dynamic Tool Visibility**（前一基线）：在 v0.8 Capability Manifest 之上增加 per-turn **`SkillSelector`**（rule-based 选 skill）+ **`ToolRouter.apply_dynamic_visibility()`**（fail-closed 动态 tool 白名单）。
...
>
> v0.8 — Capability Manifest Refactor（再前一基线）：把 Module / Skill / Tool 三层元数据**统一**为 `CapabilityManifest`，由 `CapabilityRegistry` 作为能力真相源。详见 [docs/CAPABILITY_MANIFEST_V08.md](docs/CAPABILITY_MANIFEST_V08.md)。

## Platform Runtime — Current Baseline

- **Agent Backend v0.8.2 — Result Contract Standardization**
- **HEAD**：(see git log — v0.8.2 refactor(agent): standardize result contracts)
...
| v0.6.x ~ v0.8.2 broader focused regression | **686** | 7 | 0 | 7 skipped = `RUN_LIVE_TESTS=1` live LLM tests. v0.7.1 baseline 615 + v0.8 新增 20 + v0.8.1 新增 23 + v0.8.2 新增 28. **0 failed**. |
