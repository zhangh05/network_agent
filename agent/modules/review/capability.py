...
Two tools:
  - review.item.list   (LLM-callable; return manual_review_items for an
                         artifact, with current status / user_note)
  - review.item.update  (LLM-callable; update status (pending /
...
CAPABILITY_REVIEW = CapabilityManifest(
    ...
    skills=[
        CapabilitySkillSpec(
            ...
            status="enabled",
            related_tools=["review.item.list", "review.item.update"],
            intent_patterns=[
                "人工复核", "人工审核", "复核项", "复核",
