...
CAPABILITY_REVIEW = CapabilityManifest(
    ...
    tools=[
        CapabilityToolRef(
            tool_id="system.review.item.list",
            status="enabled", callable_by_llm=True, risk_level="low",
            requires_approval=False, forbidden=False,
            handler_ref="tool_runtime.canonical_registry:_review_item_list",
            description="列出待审核项。",
        ),
        CapabilityToolRef(
            tool_id="system.review.item.update",
            status="enabled", callable_by_llm=True, risk_level="medium",
            requires_approval=True, forbidden=False,
            handler_ref="tool_runtime.canonical_registry:_review_item_update",
