...
def _build_handler(service_fn, tool_id_str: str):
    def _handler(args: dict, context=None) -> dict:
        ...
        out = tr.to_dict()
        # Back-compat top-level fields for v0.7.x consumers.
        if "status" in result:
...
tool_handler_list = _build_handler(_review_service.list_review_items, "review.list_items")
tool_handler_update = _build_handler(_review_service.update_review_item, "review.update_item")
