# agent/runtime/context_pipeline/stages.py
...
class ContextInitStage:
    ...
    def run(self, session: Any, turn: Any, **inputs) -> ContextStageResult:
        try:
            ...
            return ContextStageResult.failed(StageName.INIT, str(exc))
...
class SceneDecisionStage:
    ...
    def run(self, ctx: Any, session: Any, **inputs) -> ContextStageResult:
        try:
            ...
            return ContextStageResult(
                name=StageName.SCENE_DECISION,
                ok=True,
                data={"scene_decision": decision},
            )
        except Exception as exc:
            ctx.metadata["scene_decision_status"] = "failed"
            ctx.metadata.setdefault("context_errors", []).append(f"scene_decision: {exc!s}"[:200])
            return ContextStageResult.failed(StageName.SCENE_DECISION, str(exc))
...
class RetrievalPolicyStage:
    ...
    def run(self, ctx: Any, session: Any, **inputs) -> ContextStageResult:
        try:
            ...
            return ContextStageResult(
                name=StageName.RETRIEVAL_POLICY,
                ok=True,
                metadata={"retrieval_decision": decision.to_dict()},
            )
        except Exception:
            ctx.metadata.setdefault("context_warnings", []).append("retrieval_policy_evaluation_failed")
            return ContextStageResult.failed(StageName.RETRIEVAL_POLICY, "retrieval_policy_evaluation_failed")
...
class RuntimeStateStage:
    ...
    def run(self, ctx: Any, session: Any, **inputs) -> ContextStageResult:
        try:
            ...
            return ContextStageResult.failed(StageName.RUNTIME_STATE, str(exc))


class EvidenceStage:
    """Stage 9: Run EvidencePipeline → EvidenceBundle."""

    def run(self, ctx: Any, turn: Any, selected_skills: list, services: Any, **inputs) -> ContextStageResult:
        try:
            ...
            # Enrich retrieval decision with actual results
            _enrich_retrieval_wrapper(ctx, evidence_bundle)

            return ContextStageResult(
                name=StageName.EVIDENCE,
                ok=True,
                data={"evidence_bundle": evidence_bundle},
            )
        except Exception as exc:
            ctx.metadata["safe_context_status"] = "failed"
            ctx.metadata.setdefault("context_errors", []).append(str(exc)[:200])
            from agent.runtime.cognition.evidence_models import EvidenceBundle
            return ContextStageResult(
                name=StageName.EVIDENCE,
                ok=False,
                errors=[str(exc)[:200]],
                data={"evidence_bundle": EvidenceBundle()},
            )
...
class ToolPlanningStage:
    ...
    def run(self, ctx: Any, evidence_bundle: Any, session: Any, services: Any,
            selected_skills: list, **inputs) -> ContextStageResult:
        warnings: list[str] = []
        try:
            cap_reg = getattr(services, "capability_registry", None) if services else None
            selector = getattr(services, "skill_selector", None) if services else None
            ...
            if scene_decision is None:
                return ContextStageResult.ok_result(
                    StageName.TOOL_PLANNING,
                    metadata={"tool_planning_skipped": True, "reason": "no_scene_decision"},
                )
            ...
            if base_reg is None:
                return ContextStageResult.ok_result(
                    StageName.TOOL_PLANNING,
                    metadata={"tool_planning_skipped": True, "reason": "no_base_registry"},
                )
            ...
            warnings.append(f"skill_selector_error: {exc!r}")
            return ContextStageResult(
                name=StageName.TOOL_PLANNING,
                ok=False,
                warnings=warnings,
...
class SafeContextStage:
    ...
    def run(self, ctx: Any, evidence_bundle: Any, services: Any,
            tool_scene: dict, rule_tool_scene: dict,
            selected_visible_tools: list, selected_skills: list,
            skill_snapshot: dict, module_snapshot: dict,
            capability_registry: Any, **inputs) -> ContextStageResult:
        try:
            # Runtime snapshot
            ...
            snapshot = build_runtime_snapshot(
                ...
                skill_snap=skill_snapshot,
                module_snap=module_snapshot,
                base_enabled_skills=base_enabled,
                selected_skills=selected_skills,
                selected_visible_tools=selected_visible_tools,
                dynamic_tool_visibility=bool(tool_scene),
            )
            ...
            # Runtime state injection (LLM-visible only)
            ...
            return ContextStageResult(
                name=StageName.SAFE_CONTEXT,
                ok=True,
                data={"safe_context": safe},
            )
        except Exception as exc:
...
class MetadataWriteStage:
    ...
    def run(self, ctx: Any, session: Any,
            selected_skills: list, selected_visible_tools: list,
            tool_scene: dict, rule_tool_scene: dict, **inputs) -> ContextStageResult:
        try:
            ...
            return ContextStageResult(
                name=StageName.METADATA_WRITE,
                ok=True,
                metadata={"context_status": "pending_pipeline_finalize"},
            )
        except Exception as exc:
            ctx.metadata["context_status"] = "degraded"
            return ContextStageResult.failed(StageName.METADATA_WRITE, str(exc))


# ═══════════════════════════════════════════════════════════════════════
# Shared helpers (thin wrappers around context_builder helpers)
# ═══════════════════════════════════════════════════════════════════════
...
def _enrich_retrieval_wrapper(ctx, evidence_bundle) -> None:
    ...
    try:
        from agent.runtime.retrieval.unknown_feedback import (
            UnknownFeedback, enrich_retrieval_decision,
        )
