...
def get_file(file_id: str):
    """GET /api/files/:id?workspace_id=default"""
    ws_id = request.args.get("workspace_id", "default")
...
def delete_file(file_id: str):
    """DELETE /api/files/:id — delete record + content dir."""
    ws_id = request.args.get("workspace_id", "default")
    rec = _read_record(ws_id, file_id)
    if not rec:
        return jsonify({"ok": False, "error": "not found"}), 404
...
def get_file_content(file_id: str):
    """GET /api/files/:id/content?workspace_id=default"""
    ws_id = request.args.get("workspace_id", "default")
    rec = _read_record(ws_id, file_id)
    ...
    if not cpath.exists():
        return jsonify({"ok": False, "error": "content file missing"}), 404
