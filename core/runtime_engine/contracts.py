...
BUILTIN_CONTRACTS: dict[str, ToolContract] = {
    ...
    "inspection.manage": ToolContract(
        ...
        description="CMDB-driven device health inspection. run creates a background task; task_get tracks it; report returns the final artifact.",
        input_schema={"required": ["action"], "properties": {"action": {"type": "string", "enum": ["run", "task_list", "task_get", "task_cancel", "report"]}}},
        output_schema={},
        side_effect="mutate_local",
        risk_level="medium",
        idempotent=False,
