...
- `host.shell.exec`, `host.powershell.exec`, and `python.exec` are enabled execution ids and model-visible through canonical aliases `host.shell.exec`, `host.powershell.exec`, and `host.python.exec`; execution requires approval state matching tool and workspace.
