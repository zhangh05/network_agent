# Network Agent Tool Catalog (v3.0 clean canonical-only)
...
v3.0 tool IDs are canonical-only. There is **no** transition surface for retired tools; if a tool needs to be retired, its governance_status becomes `forbidden`.
...
|---|---|---|
| active | 98 | planner default candidate |
...
|---|---|---|---|---|---|---|
| **Agent 多 Agent** (`agent`) | 技能、子 Agent、角色、团队和结果读取。 | 子 Agent、技能、角色、团队任务编排。 | agent.spawn 受 max_turns≤3 限制；skill.load 不加载未经审查的技能。 | 6 | 11 | 11 |
...
| **Knowledge 知识库** (`knowledge`) | 知识库问答、检索、导入和索引管理。 | 知识库检索、chunk/source 维护、导入文件 / 文档。 | 不替代 Web 搜索；不返回未经脱敏全文；不删除 artifact 本体。 | 7 | 17 | 17 |
...
| **Workspace 工作区** (`workspace`) | 工作区文件、Artifact 制品和 workspace 元数据。 | 工作区文件列表 / 读取 / 编辑、artifact 元数据、安全摘要读写。 | 不跨 workspace；不绕过 artifact 安全策略；不访问绝对路径。 | 4 | 17 | 17 |
...
| `knowledge.query` | knowledge | query | `knowledge.query` | — | Direct canonical action. |
...
| `network.config.parse` | network | config | `network.config.parse` | — | Direct canonical action. |
| `network.config.translate` | network | config | `network.config.translate` | — | Offline network configuration translation. |
| `network.interface.extract` | network | interface | `network.interface.extract` | — | Direct canonical action. |
| `network.route.extract` | network | route | `network.route.extract` | — | Direct canonical action. |
...
**Groups**: `result, role, skill, slash_command, subagent, team`
...
- **usage**: Read a sub-agent's result.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: List registered agent roles.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Spawn a sub-agent under runtime limits.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Coordinate skills and child-agent work under runtime limits.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Load a skill into the active session.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Search skills by name or capability.
- **boundary**: Do not enable untrusted skills.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Unload a skill from the active session.
...
- **usage**: Run a PowerShell command on the local host. Requires approval.
...
- **risk_level**: high
- **requires_approval**: true
- **usage**: Run a Python snippet on the local host. Requires approval and AST whitelist.
- **boundary**: Do not use for network device access.
...
- **risk_level**: high
- **requires_approval**: true
- **usage**: Run a shell command on the local host. Requires approval.
- **boundary**: Do not use for network device access. Do not use to parse configuration text.
...
**Groups**: `chunk, import, not_found, parent, query, search, source`
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: List chunks for a knowledge source.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Read a knowledge chunk by id.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Import a document into the knowledge base.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Import a file into the knowledge base.
- **boundary**: Do not import unredacted secrets.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Explain why a knowledge query returned no result.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Read a knowledge parent document.
- **boundary**: Do not return unredacted full text.

### `knowledge.query`

- **display_name**: 知识库问答
- **category / group / action**: knowledge / query / query
- **capability_actions**: `knowledge.query`, `knowledge.search_and_answer`
- **governance_status**: `active`
- **planner_visible**: true
- **risk_level**: low
- **requires_approval**: false
- **usage**: Query the knowledge base and answer from safe excerpts.
- **boundary**: Do not return unredacted full text.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Search the knowledge base for relevant chunks.
- **boundary**: Do not return unredacted full text.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Delete a knowledge source.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Disable a knowledge source.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Read knowledge source metadata.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Read a knowledge source by id.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Reindex a knowledge source.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Reindex every knowledge source.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Confirm a pending memory record.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Read user profile fields.
- **boundary**: Do not include secrets.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Update user profile fields.
- **boundary**: Do not store secrets.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Offline parse of a network device configuration.
- **boundary**: Do not log into real devices. Do not generate deployable configs.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Translate a parsed configuration between formats.
- **boundary**: translated_config is NOT deployable_config.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Extract interface entries from a configuration.
- **boundary**: Do not log into real devices.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Extract route entries from a configuration.
- **boundary**: Do not log into real devices.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Summarize a CSV document.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Validate a JSON document.
- **boundary**: Do not execute embedded code.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Extract a table from text.
- **boundary**: Do not execute embedded code.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Validate a YAML document.
- **boundary**: Do not execute embedded code.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Render a Mermaid diagram.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Render a safe summary document.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Save a report as a workspace artifact.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Render a markdown report.
- **boundary**: Do not include raw sensitive config as final output.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Classify text into a safe category.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Compute a diff between two texts.
- **boundary**: Do not execute embedded code.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Redact sensitive content from text.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Update a review item's status.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Preview an archive action without executing it.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Return runtime diagnostics summary.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Return runtime health summary.
- **boundary**: Do not include sensitive trace payloads.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Preview a retention action without executing it.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Run a runtime self-check.
- **boundary**: Do not include sensitive trace payloads.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Create a session checkpoint.
- **boundary**: Do not include secrets.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Export a session to an artifact.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Rewind a session to a checkpoint.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Create a session snapshot.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Search vendor official documentation.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Run a public news search.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Extract links from a public web page.
- **boundary**: Do not query private / local / login-walled URLs.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Save a fetched web page to a workspace artifact.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Summarize a public web page.
- **boundary**: Do not query private / local / login-walled URLs.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Read current weather for a public location.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Read weather forecast for a public location.
- **boundary**: Use only when an explicit weather question is asked.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Soft-delete a workspace artifact.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Diff two workspace artifacts.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Export a workspace artifact to a destination path.
...
- **boundary**: Do not return artifact bodies; use artifact.read.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Read a workspace artifact with policy-aware redaction.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Save a new workspace artifact.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Search workspace artifacts by metadata filters.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Tag a workspace artifact.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Extract text from a PDF document in workspace.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Edit a workspace file with an explicit new content body.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Check whether a workspace file exists.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: List files in the workspace.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Apply a unified diff patch to a workspace file.
- **boundary**: Do not use for absolute paths outside the workspace.
...
- **capability_actions**: `workspace.file.manage`, `workspace.file.read`
- **governance_status**: `active`
- **planner_visible**: true
- **risk_level**: low
- **requires_approval**: false
- **usage**: Read a complete text file in the workspace.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Write a file into the workspace artifact output area.
...
- **risk_level**: low
- **requires_approval**: false
- **usage**: Read safe workspace metadata.
- **boundary**: Do not return secrets.


## 6. Governance Summary

active=98 disabled=0 internal=0 forbidden=0
