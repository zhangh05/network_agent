...
class TestWorkspaceArtifact:
    def test_state_has_artifact_counts(self, client):
        ...
        assert "artifacts_count" in state or "artifact_counts" in state or "last_input_artifacts" in state
...
class TestTraceMetadata:
    def test_trace_artifact_counts(self, client):
        ...
        assert "artifact_saved_count" in tl or data.get("ok") is False
...
class TestRegression:
    def test_translate_source_config(self, client):
        ...
        assert resp.status_code == 200
        data = resp.get_json()
        assert data.get("ok") is True or data.get("error_type") in ("missing_api_key", "provider_error", "disabled_by_user")
