...
def test_tool_call_standard_fields():
    ...
    tc = _to_standard_tool_call("call_001", "test.tool", tr)
...
def test_tool_call_dict_compat():
    """Legacy dict-shaped results also produce the 10 standard fields."""
    from agent.runtime.loop import _to_standard_tool_call
    ...
    tc = _to_standard_tool_call("call_002", "test.fail", legacy)

    assert tc["ok"] is False
    assert tc["summary"] == "something went wrong"
    assert tc["errors"] == ["bad arg"]
    # All fields present even for legacy
...
def test_agent_result_metadata_has_selected_skills():
    ...
    # This is validated through the _enrich_metadata helper
    ...
    enriched = _enrich_metadata(base, ctx)
...
def test_agent_result_metadata_no_overwrite():
    """_enrich_metadata does NOT overwrite existing keys."""
    from agent.runtime.loop import _enrich_metadata
    from types import SimpleNamespace
...
def test_workspace_counts_artifact_subdirs():
    """_count_artifacts covers all 7 subdirectories."""
