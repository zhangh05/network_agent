# harness/test_phase2_durable_state.py
...
class TestApprovalGuardUnaffected:
    """Phase 2 must not affect existing approval guard."""

    def test_approval_guard_still_importable(self):
        from agent.approval import get_approval_store, ApprovalStore
        store = get_approval_store()
        assert store is not None

    def test_approval_store_creates_record(self):
        from agent.approval import get_approval_store
        store = get_approval_store()
        ws = f"ws_ag_{uuid.uuid4().hex[:8]}"
        store.create(
            session_id="sess-1", tool_id="exec.run",
            arguments={"cmd": "ls"}, description="test",
            risk_level="high", workspace_id=ws,
        )
        pending = store.get_pending(session_id="sess-1")
        assert any(p.get("tool_id") == "exec.run" for p in pending)
