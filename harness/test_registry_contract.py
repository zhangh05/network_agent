...
class TestModuleContract:
    ...
    def test_config_module_no_retired_frontend(self):
        from registry.loader import get_module
        m = get_module("config_translation")
        assert m.has_own_retired_frontend is False
...
class TestAgentRegistryExecution:
    def test_agent_router_maps_intent_to_capability(self, client):
        ...
        data = resp.get_json()
        assert data["intent"] in ("translate_config", "assistant_chat")
    ...
    def test_agent_executor_uses_skill(self, client):
        ...
        data = resp.get_json()
        if not data.get("ok"):
            pytest.skip("LLM unavailable — skipping agent-dependent test")
        assert data["selected_skill"] in (None, "config_translation")
    ...
    def test_agent_trace_has_capability_id(self, client):
        ...
        if resp2.status_code == 200:
            ...
            if not skill_events and not resp.get_json().get("ok"):
                pytest.skip("LLM unavailable — no skill events in trace")
