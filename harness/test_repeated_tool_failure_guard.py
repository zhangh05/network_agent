def test_repeated_tool_failure_detects_same_failed_call_twice():
    ...
    assert repeated
    assert repeated["tool_id"] == "config.analysis.run"
...
def test_repeated_tool_failure_ignores_different_errors():
    ...
    results = [
        {"tool_id": "config.analysis.run", "ok": False, "errors": ["missing_source_config"]},
        {"tool_id": "config.analysis.run", "ok": False, "errors": ["translation_error"]},
    ]
