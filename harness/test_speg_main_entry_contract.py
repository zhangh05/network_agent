"""SPEG main-entry contract tests."""

from __future__ import annotations


def test_agent_app_submit_uses_speg_runtime(monkeypatch, temp_dirs):
    ...
    monkeypatch.setattr("agent.runtime.speg_adapter._invoke_llm_for_speg", fake_llm)
    ...
    assert result.final_response == "收到"
    assert result.metadata["runtime_engine"] == "speg"
    assert result.metadata["timeline_summary"]["llm_calls"] == 1
    assert result.tool_calls == []


def test_speg_tool_node_invokes_tool_runtime_client(monkeypatch, temp_dirs):
    ...
    monkeypatch.setattr("agent.runtime.speg_adapter._invoke_llm_for_speg", fake_llm)
    monkeypatch.setattr("agent.runtime.speg_adapter._tool_runtime_client", lambda: FakeClient())
