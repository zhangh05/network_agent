"""
P0 fix coverage: ``speg_engine.tool_runtime.ToolRuntime`` must propagate
...
Background (5-layer audit, v3.10):
  ...
  so the bug propagated end-to-end:
    ...
    * ``result.metadata.speg.all_nodes_success`` stays True even
...
These tests assert the new behavior:
  ...
  * The engine's ``metadata.speg`` reports ``node_failure_count=1``
...
from speg_engine.models import ExecutionStatus, SPEGConfig, ToolResult
from speg_engine.tool_runtime import ToolRuntime


def _make_runtime(handler, *, max_retries: int = 0) -> ToolRuntime:
    cfg = SPEGConfig(max_retries_per_node=max_retries)
    rt = ToolRuntime(cfg)
...
def _run_node(handler, args=None):
    ...
    from speg_engine.models import ExecutionNode, StatelessContext
...
def test_engine_metadata_reflects_node_failure():
    ...
    cfg = SPEGConfig(enable_finalizer=False, max_retries_per_node=0)
    engine = SPEGEngine(
        config=cfg,
        llm_invoke=mock_llm,
    ...
    # the top level of ``result.metadata`` (speg_adapter flattens them
