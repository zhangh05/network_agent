...
The v4 contract package is ``core.runtime_engine.runtime_contracts`` and
exposes:
...
def test_engine_run_asserts_contracts(monkeypatch):
    ...
    cfg = SSOTRuntimeConfig(enable_finalizer=False)
...
def test_normalize_result_refuses_when_contract_off():
    ...
    """
    from core.runtime_engine.tool_runtime import _normalize_result
...
def test_enforce_refuses_when_contract_off():
    ...
    """
    from core.runtime_engine.planner import enforce_execution_obligation, detect_task_intent
...
def test_contract_has_a_single_source_path():
    ...
    engine, ssot_runtime) import the contract from
    ``core.runtime_engine.runtime_contracts``. This is the meta-
    guarantee that the contract is not silently redefined
    elsewhere.
    """
    import core.runtime_engine.runtime_contracts as rc
    ...
    import core.runtime_engine.tool_runtime as tr
    assert tr.ExecutionContract is rc.ExecutionContract
    import core.runtime_engine.planner as pl
    assert pl.ExecutionContract is rc.ExecutionContract
    import core.runtime_engine.engine as en
    assert en.ExecutionContract is rc.ExecutionContract
    import agent.runtime.ssot_runtime as sa
