...
class TestWorkspaceManager:
    ...
    def test_writes_and_reads_state(self, temp_dirs):
        from storage.workspace_store import ensure_workspace, update_workspace_state, get_workspace_state
        ws_id = "state_test"
        ensure_workspace(ws_id)
    ...
    def test_list_workspaces_runs_count(self, temp_dirs):
        from storage.workspace_store import ensure_workspace, list_workspaces
        ws_id = "count_test"
        ensure_workspace(ws_id)
...
class TestRunStore:
    ...
    def test_write_run_record(self, temp_dirs):
        from storage.workspace_store import ensure_workspace
        from storage.run_record_store import write_run_record, list_runs
    ...
    def test_get_last_run(self, temp_dirs):
        from storage.workspace_store import ensure_workspace, get_workspace_runs
        from storage.run_record_store import write_run_record, get_last_run
        import uuid
