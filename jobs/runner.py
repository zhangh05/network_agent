...
def _run_translate_config(rec: JobRecord):
    ...
    append_log(ws, jid, "Running translate_config via module service")
    append_event(ws, jid, JobEvent(job_id=jid, workspace_id=ws,
                 event_type="job_run_started", message="Starting translate_config"))

    # Call config_translation module service directly (no retired graph)
    from modules.config_translation.backend.service import translate_config, TranslateRequest
    req = TranslateRequest(
        source_config=payload.pop("source_config", payload.pop("message", "")),
        source_vendor=payload.pop("source_vendor", "cisco"),
        target_vendor=payload.pop("target_vendor", "huawei"),
    )
    ...
    update_job(ws, jid, {
        ...
        "output_artifacts": result_dict.get("output_artifacts", []),
        "report_artifacts": result_dict.get("report_artifacts", []),
        "input_artifacts": result_dict.get("input_artifacts", []),
        "artifact_refs": result_dict.get("artifact_refs", []),
        "result_summary": {"ok": result_dict.get("ok"), "intent": "translate_config"},
    })
    # Record output artifact ID
    for aid in result_dict.get("output_artifacts", []):
        append_event(ws, jid, JobEvent(job_id=jid, workspace_id=ws,
                     event_type="job_artifact_saved", artifact_id=aid,
                     message=f"Output artifact: {aid}"))
    for aid in result_dict.get("report_artifacts", []):
        append_event(ws, jid, JobEvent(job_id=jid, workspace_id=ws,
...
def _run_batch_translate(rec: JobRecord):
    ...
    for i, aid in enumerate(artifact_ids):
        ...
        try:
            ...
            art = get_artifact(ws, aid)
            if not art:
                results.append({"aid": aid, "error": "artifact not found"})
                continue
            req = TranslateRequest(
                source_config=art.content or "",
                source_vendor="cisco",
                target_vendor="huawei",
            )
            result = translate_config(req)
            result_dict = result.as_dict() if hasattr(result, "as_dict") else result
            results.append({"aid": aid, "ok": result_dict.get("ok")})
            ...
            for oa in result_dict.get("output_artifacts", []):
