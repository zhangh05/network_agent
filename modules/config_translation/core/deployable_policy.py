...
class Provenance(str, Enum):
    ...
    LEGACY_STRING = "legacy_string"      # old-style string from translator
    UNKNOWN = "unknown"
...
class DeployablePolicy:
    ...
    def classify_translation(self, candidate) -> "ClassifiedTranslation":
        ...
        result = self.classify(CandidateLine(
            ...
            provenance=Provenance(candidate.provenance.value) if isinstance(candidate.provenance, TMProv) else Provenance.LEGACY_STRING,
        ...
        return ClassifiedTranslation(
            ...
            provenance=TMProv(result.provenance.value) if hasattr(result.provenance, 'value') else TMProv.LEGACY_STRING,
    ...
    def classify(self, candidate: CandidateLine) -> ClassifiedLine:
        ...
        if candidate.provenance not in (
            ...
            # Legacy string or unknown provenance → not safe for deployable
            return ClassifiedLine(
                target=DeployableDecision.MANUAL_REVIEW,
                line=line, source_line=source_line,
...
def quick_assess(line: str, source_platform: str, target_platform: str) -> tuple[Provenance, Confidence]:
    """[LEGACY FALLBACK] Quick safety assessment for legacy translator outputs.
    ...
    lower = line.strip().lower()
    if not lower:
        return Provenance.LEGACY_STRING, Confidence.NONE
