...
Design principles:
  ...
  - quick_assess() is the LEGACY fallback, NOT the primary path.
...
def review_candidate(
    ...
    candidate = TranslationCandidate(
        ...
        target_platform=to_vendor,
        module=module,
        provenance=Provenance.LEGACY_STRING,
...
def candidate_zone(stripped: str, lower: str, from_vendor: str, to_vendor: str) -> Optional[TranslationCandidate]:
    ...
    if "security-zone" in lower and to_vendor in ("hillstone", "topsec"):
        return None  # routed to legacy/semantic_near
    return None
...
def candidate_acl_binding(stripped: str, lower: str, from_vendor: str, to_vendor: str) -> Optional[TranslationCandidate]:
    """ACL binding on interface. Same-vendor: passthrough. Cross-vendor: let legacy translate."""
...
_MODULE_CANDIDATE_FUNCTIONS = [
    ...
    candidate_snmp_basic,
    candidate_ntp,
    candidate_banner,
    candidate_description_extended,
    candidate_lldp,
    candidate_lag_header,
    candidate_eth_trunk_header,
    candidate_eth_trunk_to_h3c,
    candidate_silent_interface_to_h3c,
    candidate_interface_header,
    candidate_shutdown,
    candidate_vlan,
    candidate_vlan_batch,
    candidate_l2_semantic,
    candidate_description,
    candidate_access_trunk,
    candidate_trunk_vlan,
    candidate_stp_edge,
    candidate_stp_enable,
    candidate_no_switchport,
    candidate_static_route_complex,
    candidate_acl_policy_review,
    candidate_static_route,
    candidate_vrf_header,
    candidate_ospf_bgp_header,
    candidate_ospf_bgp_network,
    candidate_neighbor_peer_base,
    candidate_zone,
    candidate_address_object,
    candidate_service_object,
    candidate_acl_header,
    candidate_acl_binding,
    candidate_undo_no,
]

# Public metadata registry — keyed by function name for test/audit access
CANDIDATE_REGISTRY = {
...
# Fields: (name, category, expected_target, risk_level, requires_context, high_risk)
...
def try_make_candidate(stripped: str, lower: str, from_vendor: str, to_vendor: str) -> Optional[TranslationCandidate]:
    ...
    for func in _MODULE_CANDIDATE_FUNCTIONS:
