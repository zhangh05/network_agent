...
def validate_module(spec: ModuleSpec, module_path: str = "") -> ValidationResult:
    ...
    # No legacy frontend
    if spec.no_legacy_frontend and spec.is_enabled():
        mp = ROOT / spec.module_path
        for legacy_dir in ["frontend", "static", "templates", "web"]:
            if (mp / legacy_dir).is_dir():
                v.add_error(f"module has legacy frontend dir: {mp/legacy_dir}")
