...
def audit():
    ...
    router = PROJECT_ROOT / "agent" / "retired" / "intent_router.py"
    ...
    executor = PROJECT_ROOT / "agent" / "retired" / "skill_executor.py"
    ...
    for py_file in PROJECT_ROOT.rglob("*.py"):
        if py_file.name.startswith("__"):
            continue
        p = str(py_file)
        if any(x in p for x in ["retired", "scripts/audit", "harness/", "registry/validator", "runtime/", "tool_runtime/"]):
            continue
        content = py_file.read_text()
        if '"/api/translate"' in content or "'/api/translate'" in content:
            if "已删除" not in content and "retired" not in p:
