...
S = {
    ...
    "session_id": {"type": "string", "description": "Session id from system.session.get or URL."},
    "run_id": {"type": "string", "description": "Run id from system.run.get_recent or trace."},
...
GENERAL_TOOL_INPUT_SCHEMAS = {
    # Artifact
    "workspace.artifact.list": _schema({"workspace_id": S["workspace_id"], "query": S["query"], "limit": S["limit"]}),
    "workspace.artifact.read": _schema({"workspace_id": S["workspace_id"], "artifact_id": S["artifact_id"]}, ["artifact_id"]),
    ...
    "knowledge.read": _schema({"workspace_id": S["workspace_id"], "source_id": S["source_id"]}, ["source_id"]),
    "knowledge.read": _schema({"workspace_id": S["workspace_id"], "chunk_id": S["chunk_id"]}, ["chunk_id"]),
    "knowledge.explain_not_found": _schema({"query": S["query"], "workspace_id": S["workspace_id"]}, ["query"]),

    # Web
    "web.page.process": _schema({"url": S["url"]}, ["url"]),
    "web.search": _schema({
        "query": S["query"],
    ...
    # Session / run / memory
    "system.session.get": _schema({"workspace_id": S["workspace_id"], "session_id": S["session_id"]}, ["session_id"]),
    ...
    "system.run.get": _schema({"workspace_id": S["workspace_id"], "limit": S["limit"]}),
    "system.run.get": _schema({"workspace_id": S["workspace_id"], "run_id": S["run_id"]}, ["run_id"]),
    "memory.search": _schema({"query": S["query"], "limit": S["limit"]}, ["query"]),
    "skill.list": _schema({"workspace_id": S["workspace_id"]}),
    "memory.manage": _schema({
        "workspace_id": S["workspace_id"],
        "title": S["title"],
        "content": S["content"],
    ...
    "memory.profile": _schema({"workspace_id": S["workspace_id"]}),
    "memory.profile": _schema({
        "workspace_id": S["workspace_id"],
    ...
    # Runtime
    "system.diagnostics": _schema({"workspace_id": S["workspace_id"]}),
    ...
    # Text / data
    "text.analyze": _schema({"text": S["text"]}, ["text"]),
    "text.analyze": _schema({"text_a": {"type": "string", "description": "First text (original/before)."}, "text_b": {"type": "string", "description": "Second text (changed/after)."}}, ["text_a", "text_b"]),
    "text.extract_keywords": _schema({"text": S["text"], "limit": S["limit"]}, ["text"]),
    "text.analyze": _schema({"text": S["text"]}, ["text"]),
    "data.validate": _schema({"text": S["text"]}, ["text"]),
    "data.validate": _schema({"text": S["text"]}, ["text"]),
    "data.csv.summarize": _schema({"text": S["text"]}, ["text"]),
    "data.table.extract": _schema({"text": S["text"]}, ["text"]),

    # Workspace
    "workspace.file.list": _schema({"workspace_id": S["workspace_id"], "subdir": {"type": "string", "description": "Workspace-relative subdirectory."}}),
    "workspace.file.read": _schema({"workspace_id": S["workspace_id"], "filepath": {"type": "string", "description": "Workspace-relative text file path."}}, ["filepath"]),
    ...
    "workspace.file.list": _schema({"workspace_id": S["workspace_id"], "filepath": S["filepath"]}, ["filepath"]),
    "workspace.get_metadata": _schema({"workspace_id": S["workspace_id"]}),

    # Approved high-risk surfaces
    "exec.run": _schema({"command": {"type": "string", "description": "Bash command. For Linux/macOS. On Windows, use powershell.exec."}}, ["command"]),
    "exec.run": _schema({"command": {"type": "string", "description": "PowerShell command. For Windows. On Linux/macOS, use shell.exec."}}, ["command"]),
    ...
    "system.session.snapshot": _schema({
        "workspace_id": S["workspace_id"],
        "session_id": S["session_id"],
        "reason": S["reason"],
    }, ["session_id"]),
    "system.session.get_snapshots": _schema({
        "workspace_id": S["workspace_id"],
        "session_id": S["session_id"],
    }, ["session_id"]),
    "system.system.session.rewind": _schema({
        "workspace_id": S["workspace_id"],
        "session_id": S["session_id"],
    ...
    # Session
    "system.system.session.checkpoint": _schema({
        "workspace_id": S["workspace_id"],
        "session_id": S["session_id"],
        "reason": S["reason"],
    }, ["session_id"]),
    "system.system.session.export": _schema({
        "workspace_id": S["workspace_id"],
        "session_id": S["session_id"],
...
def _default_tool_next_actions(tool_id: str, *, ok: bool) -> list[str]:
    ...
    defaults = {
        "artifact": ["如需正文，下一步用 workspace.artifact.read 读取 artifact_id；如需索引到知识库，用 knowledge.index_artifact。"],
        "knowledge": ["根据返回的 chunk/source id 继续调用 knowledge.read 或 knowledge.read。如无结果，尝试 workspace.artifact.list、web.search 或让用户上传资料。"],
        "web": ["用返回的 citation/URL 回答并引用来源。需要正文细节时调用 web.page.process。如 web.search 无结果，尝试 web.official_doc_search 或让用户提供 URL。"],
        "weather": ["直接使用 current/forecast_daily 字段回答，并引用来源 URL 和更新时间。"],
        "news": ["交叉比较前几条来源的发布时间和作者，避免把单一网页当最终事实。区分官方公告与媒体报道。"],
        "session": ["如需展开某条记录，继续调用对应 get_summary 工具。如找不到 session，检查 system.session.get。"],
        "run": ["如需展开某条运行记录，继续调用 run.get_summary。如找不到 run，检查 system.run.get_recent。"],
...
# ── A. Artifact Tools ──
_reg("workspace.artifact.list", "Artifact Search", "artifact", "low",
...
_reg("workspace.artifact.read", "Read Content Safe", "artifact", "low",
     "Read a size-limited safe preview of artifact content. Use ONLY after workspace.artifact.list/list identifies an artifact_id. Sensitive artifacts return metadata-only. Not for: reading raw files (use file.read). Read-only. Returns text preview or metadata block.", handle_artifact_read_content_safe, permission_action="read")
...
_reg("knowledge.read", "Get Source", "knowledge", "low",
...
_reg("knowledge.read", "Get Chunk Summary", "knowledge", "low",
...
_reg("web.page.process", "Fetch Summary", "web", "medium",
     "Fetch and summarize a public http(s) webpage. Use after web.search or when user provides a URL. Blocks private/local URLs (localhost, 192.168.x, 10.x). Not for: workspace files (use file.read), artifacts (use workspace.artifact.read). Returns URL, title, and text summary. Medium risk (network call).", handle_web_fetch_summary, permission_action="network")
_reg("web.search", "Official Doc Search", "web", "low",
...
_reg("system.session.get", "List Sessions", "session", "low",
...
_reg("system.session.get", "Session Summary", "session", "low",
...
_reg("system.run.get", "Recent Runs", "session", "low",
...
_reg("system.run.get", "Run Summary", "session", "low",
...
_reg("memory.manage", "Create Memory", "memory", "low",
...
_reg("memory.search", "List Memories", "memory", "low",
...
_reg("memory.profile", "Get Profile", "memory", "low",
...
_reg("memory.profile", "Set Profile", "memory", "low",
...
_reg("memory.manage", "Confirm Memory", "memory", "low",
...
# ── E. Runtime Tools ──
_reg("system.diagnostics", "Runtime Health", "runtime", "low",
...
_reg("system.diagnostics", "Diagnostics", "runtime", "low",
...
# ── G. Text / Data Tools ──
_reg("text.analyze", "Redact Text", "text", "low",
...
_reg("text.analyze", "Text Diff", "text", "low",
...
_reg("text.analyze", "Classify Text", "text", "low",
...
_reg("data.validate", "Validate JSON", "text", "low",
...
_reg("data.validate", "Validate YAML", "text", "low",
...
_reg("data.csv.summarize", "CSV Summarize", "text", "low",
...
_reg("data.table.extract", "Extract Table", "text", "low",
...
_reg("workspace.file.read", "Read Text Preview", "workspace", "low",
...
_reg("workspace.file.list", "Path Exists", "workspace", "low",
...
_reg("exec.run", "Shell Exec", "shell", "high",
...
_reg("exec.run", "PowerShell Exec", "powershell", "high",
...
_reg("system.session.snapshot", "Session Snapshot", "session", "low",
     "Create a snapshot of the current session's messages for later recovery or rewind. Use when: user wants to checkpoint before a large operation or enable rollback. Read-only for current state (writes snapshot). Returns snapshot_id. Use with system.system.session.rewind to restore.", handle_session_snapshot, permission_action="read")
_reg("system.session.get_snapshots", "List Snapshots", "session", "low",
...
_reg("system.system.session.rewind", "Session Rewind", "session", "medium",
...
_reg("workspace.file.list", "File Exists", "file", "low",
...
_reg("memory.manage", "Update Memory", "memory", "medium",
     "Update an existing memory entry's content. Checks for secrets before writing. Use when: user wants to modify a stored preference or fact. Medium risk (modifies persistent memory). Requires memory_id from memory.search or memory.search. Returns confirmation.", handle_memory_update, permission_action="write")
_reg("memory.manage", "Soft Delete Memory", "memory", "medium",
...
_reg("system.system.session.checkpoint", "Session Checkpoint", "session", "low",
...
_reg("system.system.session.export", "Export Session", "session", "low",
